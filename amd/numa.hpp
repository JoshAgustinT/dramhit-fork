...
class NumaPolicyThreads : public Numa {
 ...
 private:
  ...
  void generate_cpu_list() {
    // 1. Get every single available CPU in the system regardless of node
    ...
    // 2. Sort them to ensure predictable assignment (0, 1, 2, 3...)
    std::sort(all_cpus.begin(), all_cpus.end());

    // 3. Ensure we aren't asking for more threads than the hardware has
    assert(this->config_num_threads == all_cpus.size());

    // 4. Just take the first N cpus
    for (uint32_t i = 0; i < this->config_num_threads; i++) {
        uint32_t cpu_assigned = all_cpus[i];
