...
void distribute_mem_to_nodes(void *addr, uint64_t alloc_sz, numa_policy_threads policy) {
  ...
  else if (policy == THREADS_SPLIT_EVEN_NODES)
  {
    ...
    printf("JOSH SPLITTING MEMORY EVENELY>>>\n\n");
    ...
    for (int i = 0; i < num_nodes; i++) {
        ...
        uint64_t current_sz = (i == num_nodes - 1) ? (alloc_sz - (i * chunk_sz)) : chunk_sz;
        ...
        if (ret < 0) {
            PLOGE.printf("mbind failed for node %d: %s (addr: %p, sz: %lu)", 
                         i, strerror(errno), chunk_ptr, current_sz);
        } else {
