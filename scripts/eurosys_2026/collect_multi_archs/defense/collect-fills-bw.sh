...
# DRAMBLAST: has bucketization, simd, uniform probing, double prefetch, no inline
# cmake -S $HOME_DIR -B $HOME_DIR/build -DIN_ORDER_BATCHING=OFF -DDRAMHiT_VARIANT=2025 -DBUCKETIZATION=ON -DBRANCH=simd -DUNIFORM_PROBING=OFF -DPREFETCH=DOUBLE
# cmake --build $HOME_DIR/build

#     echo "START dramblast_no_inline {" >> $OUTPUT_FILE
#     cmd="--perf_cnt_path ./perf_cnt.txt --perf_def_path ./perf-cpp/perf_list.csv \
#     --find_queue 64 --ht-fill $fill --ht-type $DRAMHIT --insert-factor $insertFactor --read-factor $readFactor\
#     --num-threads $numThreads --numa-split $numa_policy --no-prefetch 0 --mode $ZIPFIAN --ht-size $size --skew 0.01\
#     --hw-pref 0 --batch-len 16 --relation_r_size $rsize"
...
#     echo "} END uniform test" >> $OUTPUT_FILE
#     echo "} END dramblast_no_inline" >> $OUTPUT_FILE





# DRAMBLAST: has bucketization, simd, uniform probing, single prefetch, no inline
# cmake -S $HOME_DIR -B $HOME_DIR/build -DIN_ORDER_BATCHING=OFF -DDRAMHiT_VARIANT=2025 -DBUCKETIZATION=ON -DBRANCH=simd -DUNIFORM_PROBING=OFF -DPREFETCH=L1
# cmake --build $HOME_DIR/build

#     echo "START dramblast_no_double {" >> $OUTPUT_FILE
#     cmd="--perf_cnt_path ./perf_cnt.txt --perf_def_path ./perf-cpp/perf_list.csv \
#     --find_queue 64 --ht-fill $fill --ht-type $DRAMHIT --insert-factor $insertFactor --read-factor $readFactor\
#     --num-threads $numThreads --numa-split $numa_policy --no-prefetch 0 --mode $ZIPFIAN --ht-size $size --skew 0.01\
#     --hw-pref 0 --batch-len 16 --relation_r_size $rsize"
...
#     echo "} END uniform test" >> $OUTPUT_FILE
#     echo "} END dramblast_no_double" >> $OUTPUT_FILE




# DRAMBLAST: no buckets, branched, uniform probing, single prefetch, no inline
cmake -S $HOME_DIR -B $HOME_DIR/build -DIN_ORDER_BATCHING=OFF -DDRAMHiT_VARIANT=2025 -DBUCKETIZATION=OFF -DBRANCH=branched -DUNIFORM_PROBING=OFF -DPREFETCH=L1
cmake --build $HOME_DIR/build

    echo "START dramblast_no_simd {" >> $OUTPUT_FILE
    ...
    echo "} END uniform test" >> $OUTPUT_FILE
    echo "} END dramblast_no_simd" >> $OUTPUT_FILE
