...
# Define the 6 Build Configurations
declare -a build_configs=(
    "-DDRAMHiT_VARIANT=2025 -DBUCKETIZATION=OFF -DBRANCH=branched -DUNIFORM_PROBING=OFF -DPREFETCH=L1"
    "-DDRAMHiT_VARIANT=2025 -DBUCKETIZATION=ON -DBRANCH=branched -DUNIFORM_PROBING=OFF -DPREFETCH=L1"
    "-DDRAMHiT_VARIANT=2025 -DBUCKETIZATION=ON -DBRANCH=simd -DUNIFORM_PROBING=OFF -DPREFETCH=L1"
    "-DDRAMHiT_VARIANT=2025 -DBUCKETIZATION=ON -DBRANCH=simd -DUNIFORM_PROBING=OFF -DPREFETCH=DOUBLE"
    "-DDRAMHiT_VARIANT=2025_INLINE -DBUCKETIZATION=ON -DBRANCH=simd -DUNIFORM_PROBING=OFF -DPREFETCH=DOUBLE"
    "-DDRAMHiT_VARIANT=2025_INLINE -DBUCKETIZATION=ON -DBRANCH=simd -DUNIFORM_PROBING=ON -DPREFETCH=DOUBLE"
)

# Define labels for the log file
declare -a labels=("base" "base+bucket" "base+bucket+simd" "base+bucket+simd+double" "base+bucket+simd+double+inline" "all+uniform")

for i in "${!build_configs[@]}"; do
    echo "[+] Building variant: ${labels[$i]}"
    
    # 1. Build
    cmake -S $HOME_DIR -B $HOME_DIR/build -DIN_ORDER_BATCHING=OFF ${build_configs[$i]} > /dev/null
    cmake --build $HOME_DIR/build --parallel > /dev/null

    # 2. Log Start
    echo "START ${labels[$i]} {" >> $OUTPUT_FILE
    echo "START uniform test {" >> $OUTPUT_FILE

    # 3. Execution Loop (10-90%)
    for fill_loop in $(seq 10 10 90); do
        # Note: --ht-type 3 for dramblast style, adjust if code expects 8
        cmd="--perf_cnt_path ./perf_cnt.txt --perf_def_path ./perf-cpp/perf_list.csv \
        --find_queue 64 --ht-fill $fill_loop --ht-type 3 --insert-factor $insertFactor --read-factor $readFactor \
        --num-threads $numThreads --numa-split $numa_policy --no-prefetch 0 --mode $ZIPFIAN --ht-size $size \
        --skew 0.01 --hw-pref 0 --batch-len 16 --relation_r_size $rsize"

        echo "[*] Running ${labels[$i]} at ${fill_loop}% fill..."
        
        # Execute and capture results
        result=$(sudo $HOME_DIR/build/dramhit $cmd | grep "get_cycles")
        echo "$result" >> $OUTPUT_FILE
        echo "$HOME_DIR/build/dramhit $cmd" >> $OUTPUT_FILE
    done

    echo "} END uniform test" >> $OUTPUT_FILE
    echo "} END ${labels[$i]}" >> $OUTPUT_FILE
done
