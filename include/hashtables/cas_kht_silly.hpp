...
//     // josh
    for (auto &data : kp) {
      // Cast to the specified InsertFindArgument type
...
// #ifdef BUCKETIZATION
//       // Apply the cacheline mask for bucket alignment
//       idx = idx - (size_t)(idx & KEYS_IN_CACHELINE_MASK);
// #endif
      // Issue the prefetch for the calculated index
      prefetch_read(idx);
    }

    for (auto &data : kp) {
      // 1. Execute the lookup using the logic from find_noprefetch
       this->insert_noprefetch(&data, collector);
      // values.second[values.first].value = (kmercounter::value_type)result;
      // values.first++;
    }
    return;
...
// #endif
    ...
    for (auto &data : kp) {
      // 1. Execute the lookup using the logic from find_noprefetch
      void *result = this->find_noprefetch(&data, collector);
      values.second[values.first].value = (kmercounter::value_type)result;
      values.first++;
    }
    return;
